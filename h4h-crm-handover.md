...
**Revision 3 reconciliation (2026-05-27):** Sessions 1, 1.5, and 2 are now shipped. The actually-shipped 1.5 drifted from the Rev 2 plan — it covered the schema floor needed to unblock the Session 2 backfill (activity_type CHECK enum extension, cents-only amount column, `payment_processor` column for the `revenue_by_processor` financial breakdown, prize_id linkage on auction activities, tax-receipt flags) but did NOT build the outbound_links table or any of the link-attribution columns on contact_activities, and did NOT add the Rev 2 `payment_method` reconciliation column (`payment_processor` and `payment_method` are distinct columns serving different reporting questions — see "What's actually shipped" below). The link-tracking schema and the new `payment_method` column are deferred to Session 1.6 (described below), and the 35 pre-launch Stripe Payment Link rows are split out as Session 2.5, which depends on 1.6 shipping first. Session 2 itself shipped with a dynamic preflight gate (±5% drift tolerance against live source counts) in place of the original static row-count band.
...
- `payment_processor` shipped on `contact_activities` (TEXT, free-form, values like `'stripe' | 'square' | 'manual' | 'cash'`). It answers *"which third-party processor handled the money"* and is wired into `get_financial_summary`'s `revenue_by_processor` JSONB aggregation. The backfill writes `payment_processor: 'stripe'` on every row.
- The Rev 2 `payment_method` column (intended as the reconciliation enum: stripe / cash / cheque / eft / in_kind / other — i.e. *how funds arrived at the event*) was NOT shipped. **These are two different columns serving two different reporting questions.** Earlier framings of Rev 3 called this a rename; that was wrong. Session 1.6 adds `payment_method` as a new, distinct column and leaves `payment_processor` and `get_financial_summary` untouched.
...
2. **Add the six missing link-tracking columns on `contact_activities`**: `outbound_link_id` (FK to outbound_links), `sent_to_contact_id` (FK to contacts), `payment_method` (TEXT with CHECK enum: stripe / cash / cheque / eft / in_kind / other; nullable so Session 2-backfilled rows stay valid — a future session can backfill `payment_method='stripe'` for them), `payment_reference` (TEXT), `entered_manually` (BOOLEAN NOT NULL DEFAULT false), `entered_by` (FK to auth.users). The existing `payment_processor` column stays as-is and `get_financial_summary` is NOT modified.
