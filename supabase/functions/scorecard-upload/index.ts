...
// PUT /scorecard-upload?token=UUID&filename=scorecard.jpg
...
Deno.serve(async (req) => {
  ...
  const filename = url.searchParams.get('filename') || 'scorecard.jpg'
  ...
  // Validate token by looking up the registration
  ...
  const { data: urlData } = supabase.storage.from('scorecard-photos').getPublicUrl(path)
